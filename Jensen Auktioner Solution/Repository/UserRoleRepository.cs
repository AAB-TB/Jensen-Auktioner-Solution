        ...
        public async Task<IEnumerable<UserRoleDto>> GetAllUsersWithRolesAsync()
        {
            try
            {
                using (var connection = _dapperContext.GetDbConnection())
                {
                    ...
                    // Execute the stored procedure to fetch users with roles
                    var usersWithRoles = await connection.QueryAsync<UserRoleDto>("sp_GetAllUsersWithRoles", commandType: CommandType.StoredProcedure);
